...
class NoteListActivity : AppCompatActivity(), View.OnClickListener {
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    when (requestCode) {
      ...
      NoteDetailActivity.REQUEST_CODE_EDIT_NOTE -> processEditNoteResult(data);
  ...
  private fun processEditNoteResult(data: Intent) {
    val index = data.getIntExtra(NoteDetailActivity.EXTRA_INDEX, -1);
  ...
  fun editNewNote(view : View) = showNoteDetail( -1);
}
